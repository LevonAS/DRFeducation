...
class TODOForm extends React.Component {
    constructor(props) {
        super(props)
        console.log("tF_0_uSS", props.projects, props.usersSS)
        this.state = { 
        text: '', 
        project: props.projects[0]?.id, 
        // creator: props.usersSS[0]?.uuid 
    } 
    console.log("tF_1_uSS", this.state)
    } 
    handleChange(event) {
        this.setState(
            {
    ...
    render() { console.log("tF_2_uSS", this.props.usersSS)
        return ( 
            <form onSubmit={(event) => this.handleSubmit(event)}>
                ...
                {/* <div className="form-group" > 
                    ...
                    <select  name="creator"  className='form-control' onChange={(event) => this.handleChange(event)}> {this.props.usersSS.map((item) => <option value={item.uuid}>{item.username}</option>)} </select>
                </div> */}
